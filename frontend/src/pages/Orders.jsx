...
export default function Orders() {
  ...
  useEffect(() => {
    load();
    // Swaps move from "processing" to "completed" asynchronously in the
    // background, so poll briefly to reflect that without a manual refresh.
    const interval = setInterval(load, 4000);
    return () => clearInterval(interval);
  }, []);

  if (orders === null) return <div className="page container loading">Loading swaps...</div>;

  return (
    <div className="page container">
      <h1 className="page-title">Your swaps</h1>
